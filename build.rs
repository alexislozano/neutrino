...
fn themes(out_dir: &str) {
    ...
    match fs::read_dir("src/www/themes") {
        ...
        Ok(entries) => for entry in entries {
            ...
            impl_data.push_str(&format!(r#"Theme::{} => "{}","#,
                &filestem, css_str
            ))
        }
    };
    ...
    match fs::read_dir("src/www/icons/") {
        ...
        Ok(dirs) => for dir in dirs {
            ...
            let mut impl_function_data = r#"fn data(&self) -> String { 
                match self {
            "#.to_string();
            let mut impl_function_extension = r#"fn extension(&self) -> String { 
                match self {
            "#.to_string();
            ...
            match fs::read_dir(&path) {
                ...
                Ok(entries) => for entry in entries {
                    ...
                    let filestem = path.file_stem().unwrap().to_str().unwrap().to_string();

                    enum_data.push_str(&format!(r#"{},"#, &filestem));

                    let extension = path1.extension().unwrap().to_str().unwrap();
                    let data = encode(&String::from_utf8_lossy(&fs::read(path2).unwrap()).replace("\n", ""));
                    
                    impl_function_extension.push_str(&format!(r#"{}Icon::{} => "{}".to_string(),"#,
                        dirstem, filestem, &extension 
                    ));

                    impl_function_data.push_str(&format!(r#"{}Icon::{} => "{}".to_string(),"#,
