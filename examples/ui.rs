...
fn main() {
    ...
    // UI 3
    let mut ui3 = Container::new().direction(Direction::Vertical).position(Position::Between).stretch();

    let b3 = Button::new("button3").text("Button").stretch();
    ui3.add(Box::new(b3));
    
    // UI 4
    let mut ui4 = Container::new().direction(Direction::Vertical).position(Position::End).stretch();
    let mut t4 = Container::new().direction(Direction::Horizontal);

    let b4 = Button::new("button4").text("Button").stretch();
    t4.add(Box::new(b4));
